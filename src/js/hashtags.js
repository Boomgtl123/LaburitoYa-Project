...
async function obtenerTodasLasPublicaciones() {
  try {
    console.log('📡 Cargando posts desde Firebase para hashtags...');
    ...
    console.log(`✅ ${posts.length} posts cargados para análisis de hashtags`);
    return posts;
  } catch (error) {
...
async function actualizarTendencias() {
  ...
  if (!trendsList) {
    console.log('No se encontró elemento trendsList');
    return;
  }
  ...
  try {
    ...
    console.log('Obteniendo hashtags populares...');
    const hashtags = await obtenerHashtagsPopulares(10);
    console.log('Hashtags obtenidos:', hashtags);
...
// Sugerencias de hashtags mientras se escribe (con debouncing)
function mostrarSugerenciasHashtags(texto) {
  ...
  debounceTimer = setTimeout(() => {
    mostrarSugerenciasHashtagsInterno(texto);
  }, 300);
}

// Función interna para mostrar sugerencias
function mostrarSugerenciasHashtagsInterno(texto) {
